...
{if(isset($_GET['sotrang'])){
   ...
   $vitri=($sotrang-1)*$bghi;
    ...
    FROM phim p
    INNER JOIN loaiphim ON loaiphim.id = p.id_loai where 1 order by p.id asc limit $vitri,$bghi";
    $re = pdo_query($sql);
    return $re;
}
function loadall_phim_hot()
{
    $sql = "SELECT p.id,p.tieu_de,p.thoi_luong_phim,p.id_loai,p.img, loaiphim.name as loaiphim 
    FROM phim p 
    INNER JOIN loaiphim ON p.id_loai = loaiphim.id 
    WHERE p.hot = 1 LIMIT 6 ";
